...
class ToDoTasks(models.Model):
    ...
    is_this_ticket = fields.Boolean(string='Is This Ticket')
    ...
    ticket_owner_id = fields.Many2one('res.users', string='Ticket Owner', readonly=1)
    ...
    def _compute_get_to_do_workers(self):
        res_user = self.env['res.users'].search([('id', '=', self.env.user.id)])
        if res_user.has_group('to_do.to_do_workers'):
            self.to_do_workers = True
        else:
    ...
    def _compute_get_account_head(self):
        ...
        if res_user.has_group('to_do.to_do_organizer'):
            ...
            self.user_accounts_head = True
        elif res_user.has_group('to_do.to_do_heads'):
            self.user_accounts_head = True
            print('ya')
        else:
